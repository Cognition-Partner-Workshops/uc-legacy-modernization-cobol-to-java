...
public final class CobolDecimal {
  ...
  public static BigDecimal decodeZoned(String encoded, int scale) {
    ...
    if (!digits.chars().allMatch(Character::isDigit)) throw new IllegalArgumentException("invalid zoned decimal digits");
  ...
  public static String encodeZoned(BigDecimal value, int integralDigits, int scale) {
    ...
    if (digits.length() > integralDigits + scale) throw new IllegalArgumentException("value exceeds PIC width");
    digits = "0".repeat(integralDigits + scale - digits.length()) + digits;
    int last = digits.charAt(digits.length() - 1) - '0';
    char overpunch = negative ? (last == 0 ? '}' : (char) ('J' + last - 1))
  ...
  public static BigDecimal decodeComp3(byte[] packed, int scale) {
    ...
    int sign = last & 0xf;
    if (sign != 0xc && sign != 0xd && sign != 0xf) throw new IllegalArgumentException("invalid COMP-3 sign");
  ...
  public static byte[] encodeComp3(BigDecimal value, int digits, int scale) {
    ...
    result[result.length - 1] = (byte) (((nibbles.charAt(nibbles.length() - 1) - '0') << 4)
