package com.suitecrm.activity.repository;

import com.suitecrm.activity.entity.Meeting;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.Optional;
import java.util.UUID;

@Repository
public interface MeetingRepository extends JpaRepository<Meeting, UUID> {
    Optional<Meeting> findByIdAndDeletedFalse(UUID id);
    Page<Meeting> findByDeletedFalse(Pageable pageable);
    Page<Meeting> findByAssignedUserIdAndDeletedFalse(UUID assignedUserId, Pageable pageable);
    Page<Meeting> findByStatusAndDeletedFalse(String status, Pageable pageable);
}
